...
def adenv(a_dur: float, d_dur: float, N: int, n: int = 0) -> np.ndarray:
    ...
    The envelope rises linearly from 0 to 1 over ``[0, a_dur]``, falls back
    from 1 to 0 over ``[a_dur, a_dur + d_dur]``, and is 0 everywhere else.
    ``a_dur`` and ``d_dur`` are in seconds; ``N`` and ``n`` are in samples.

    Returns an array of shape ``(N, 1)`` so the envelope broadcasts across
    the channels of an :class:`pq.Audio` buffer when multiplied.
    """
    t = (n + np.arange(N)) / F_S
    env = np.interp(
