This repository hosts the code for a computer music textbook called _Principles of Computer Music_ (PCM). This is intended to be a technically rigorous textbook used when music is taught in a computer science context.
...
- Each chapter should conclude w/ some :::{exercise} questions for students to complete and turn in to their instructor. Most often, these should be concrete questions that ask students to apply the technical knowledge they've learned from the chapter. Sometimes, these can be open-ended (e.g., for chapter 0). It's rare that I would want these to be coding challenges - these will be addressed in a separate part of the class. Instead, they should be consistent with the types of applied knowledge questions (not memorization) that I might want to ask students about on exams.
  - An exercise can optionally carry its own solution: nest an unlabeled `:::{solution}` inside the `:::{exercise}` (see `directives/directives-markdown.md`), and the reader gets a "Reveal solution" button that keeps the answer hidden until clicked. Add solutions to exercise questions in scenarios where there is a single correct answer. Don't show work in the solutions - students will learn better if they need to work through the solutions themselves.
...
- Figures and sound examples:
  ...
  - Most chapters should end with a `## Musical examples` section pointing to real pieces of music that connect to the chapter. Format each example as its own `### Artist - _Title_ (Year)` subsection, followed by a short paragraph describing the piece and why it is relevant to the chapter and/or significant in general, followed by an embedded YouTube player. Embed the player with a plain YouTube `<iframe>` (no directive), using the privacy-friendly `https://www.youtube-nocookie.com/embed/VIDEO_ID` URL, and append `?start=SECONDS` to jump to a relevant timestamp when appropriate. Drop the `si=` share-tracking parameter from provided links.
- Code
